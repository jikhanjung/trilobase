...
### 접근 방식: Query Parameter 기반 패키지 선택

URL prefix (`/packages/{name}/api/...`)보다 query parameter (`/api/manifest?package=trilobase`)가 기존 엔드포인트 변경을 최소화한다. 프론트엔드에서 선택한 패키지를 세션 스토리지에 저장하고 모든 API 호출에 포함.
...
│  └─────────────────────────────────┘    │
│  ↓ fetch('/api/manifest?package=X')     │
└─────────────────────────────────────────┘
         ↓
┌─────────────────────────────────────────┐
│  FastAPI (single instance)              │
│                                         │
│  Middleware: extract ?package=X          │
│  → request.state.package_name = X       │
│                                         │
│  get_db_for_request(request)            │
│  → registry.get_db(X)                   │
│                                         │
│  Endpoints: unchanged logic,            │
│  just use get_db_for_request()          │
└─────────────────────────────────────────┘
         ↓
┌─────────────────────────────────────────┐
│  PackageRegistry                        │
...
GET /api/manifest?package=X    → 기존과 동일 (X 패키지의 manifest)
GET /api/queries/{name}/execute?package=X  → 기존과 동일
...기타 모든 엔드포인트에 ?package=X 추가
```

`?package` 생략 시: `_active_package_name` fallback (단일 패키지 모드 호환).

### 프론트엔드 변경

1. **Package Selector UI**: 상단 네비게이션에 패키지 선택 드롭다운
2. **세션 스토리지**: 선택한 패키지를 `sessionStorage`에 저장
3. **API fetch wrapper**: 모든 fetch 호출에 `?package=X` 자동 추가
...
1. `app.py`에 FastAPI dependency 추가: `get_current_package(request)` → `request.query_params.get('package')` or fallback
2. `get_db()` 호출을 `get_db_for_request(request)` 패턴으로 교체 (또는 middleware)
3. `GET /api/packages` 엔드포인트 추가
4. 기존 엔드포인트에 `?package=X` 지원 (생략 시 기존 동작 유지)

### Phase 2: Frontend — Package Selector

1. `/api/packages` 호출하여 사용 가능한 패키지 목록 획득
2. 상단 UI에 패키지 선택 드롭다운/탭 추가
3. API fetch 래퍼에 `?package=X` 자동 첨부
4. 패키지 전환 시 manifest 재로딩 + 뷰 초기화
...
### 하위 호환성
- `?package` 파라미터 생략 시 기존 단일 패키지 모드로 동작
...
### 보안 (Docker)
- `?package=X`의 X를 registry에 등록된 이름으로만 제한 (path traversal 방지)
