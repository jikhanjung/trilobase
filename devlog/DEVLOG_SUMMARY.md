...
**최종 업데이트:** 2026-03-03
**총 문서:** 201개 (작업 로그 115개, 계획 문서 81개, 리뷰 4개, 기타 1개)
**개발 기간:** 2026-02-04 ~ 2026-03-02 (27일)

> **참고:** 02-18 이전 문서(001~077, P01~P60, R01~R02)는 `devlog/archive/`에 보관되어 있습니다.
> 02-19 이후 문서(078~107, P62~P79, R01~R02)는 `devlog/`에 위치합니다.
...
### 2026-02-27 (Day 23) — Synonym → Taxonomic Opinions 마이그레이션

| # | 유형 | 제목 | 문서 |
|---|------|------|------|
| P70 | 계획 | Synonyms → Taxonomic Opinions Migration Plan | `20260227_P70_synonym_migration_plan.md` |
| 096 | 작업 | T-4: Synonyms → Taxonomic Opinions Migration | `20260227_096_synonym_migration.md` |
| 097 | 작업 | Post-Migration Fix: Synonym Manifest & Fide Linking | `20260227_097_synonym_manifest_and_fide_fix.md` |

**결과:**
- 1,055 synonym 레코드를 `taxonomic_opinions`로 마이그레이션 (SYNONYM_OF)
- `synonyms` 테이블을 backward-compatible VIEW로 전환
- `taxon_bibliography.synonym_id` → `opinion_id` 리네이밍
- taxonomic_opinions: 84→1,139건 (SYNONYM_OF 1,055 + PLACED_IN 82 + SPELLING_OF 2)
- fide→bibliography 매칭 추가 133건 (총 566건)
- genus_detail Synonymy 섹션 렌더링 수정 (linked_table 전환)
- 테스트 101→108 (+7), Trilobase v0.2.4

---

### 2026-02-28 (Day 24) — 데이터 대수술 + Rebuild Pipeline + Assertion DB

프로젝트 최대 규모의 하루. country_id/formation 대수정, 소스→DB 완전 재구성 파이프라인 구축, assertion-centric 테스트 DB 생성, Treatise PDF taxonomy 추출까지 5개 작업 + 8개 계획 문서.

| # | 유형 | 제목 | 문서 |
|---|------|------|------|
| P71 | 계획 | country_id 수정 + Formation 오정렬 해결 | `20260228_P71_fix_country_id_and_formation_plan.md` |
| P72 | 계획 | 소스 텍스트 → DB 완전 재구성 파이프라인 설계 | `20260228_P72_rebuild_pipeline_plan.md` |
| P73 | 계획 | Rebuild DB ↔ Reference DB 차이 해소 | `20260228_P73_rebuild_diff_resolution_plan.md` |
| P74 | 계획 | Assertion-Centric Test DB | `20260228_P74_assertion_centric_plan.md` |
| P74b | 계획 | Assertion DB UI 기능 동등화 + Profile 표시 | `20260228_P74b_assertion_ui_parity_plan.md` |
| P75 | 계획 | Radial Taxonomy Tree Visualization | `20260228_P75_radial_tree_visualization_plan.md` |
| 098 | 작업 | T-5: country_id 수정 + Formation 오정렬 해결 | `20260228_098_fix_country_id_and_formation.md` |
| 099 | 작업 | Modular Rebuild Pipeline 완성 (35/35 검증) | `20260228_099_rebuild_pipeline_complete.md` |
| 100 | 작업 | Rebuild DB ↔ Reference DB 차이 해소 | `20260228_100_rebuild_diff_resolution.md` |
| 101 | 작업 | P74b Assertion DB UI 기능 동등화 | `20260228_101_P74b_assertion_ui_parity.md` |
| 102 | 작업 | Treatise Chapter 4 & 5 Taxonomy Tree 추출 | `20260228_102_treatise_taxonomy_extraction.md` |

**결과:**
- **T-5 (098)**: country_id 3,769건 수정 (77.8%), formation 오정렬 350건 수정, region_id 272건 수정, location 314건 복원. genus_formations 4,853→4,503, genus_locations 4,841→4,849. v0.2.5
- **Rebuild (099)**: 8단계 모듈화 파이프라인 (`scripts/pipeline/`) 완성. 소스 텍스트 2개에서 DB 완전 재현. 35/35 검증 통과
- **Diff 해소 (100)**: rebuild vs reference DB 행 단위 차이 해소. Fix A~H 수정, Fix F/I 문서화. SYNONYM_OF diff 0건
- **Assertion DB (101)**: P74 assertion-centric DB에 junction table 복사, UI queries 13→40, manifest views 6→15, PaleoCore 의존성, classification profile 표시. 15/15 검증
- **Treatise 추출 (102)**: Treatise Ch4 (Agnostida) + Ch5 (Redlichiida) PDF에서 taxonomy JSON 추출
- 테스트 108→112 (+4), Trilobase v0.2.5→v0.2.6

---

### 2026-03-01 (Day 25) — Radial Tree + Versioned DB + Treatise Import + CRUD

Assertion DB 고도화의 집약일. Radial Tree 시각화를 assertion/canonical 양쪽에 구현, DB 파일명 버전화, Treatise 데이터 import, profile selector UI, CRUD 프레임워크 적용까지 6개 작업 + 4개 계획 문서.

| # | 유형 | 제목 | 문서 |
|---|------|------|------|
| P76 | 계획 | Radial Tree → Canonical trilobase.scoda | `20260301_P76_radial_tree_canonical_scoda.md` |
| P77 | 계획 | Canonical DB 파일명에 버전 포함 | `20260301_P77_versioned_db_filename.md` |
| P78 | 계획 | Treatise (2004) Taxonomy Import Plan | `20260301_P78_treatise_import_plan.md` |
| P79 | 계획 | Profile-Based Taxonomy Tree + Profile Selector UI | `20260301_P79_profile_selector_plan.md` |
| 102b | 작업 | P75 Radial Tree — Assertion DB 구현 + scoda-engine 고도화 | `20260301_102b_P75_radial_tree_assertion_impl.md` |
| 103 | 작업 | P76 Radial Tree → Canonical trilobase.scoda 구현 | `20260301_103_P76_radial_tree_canonical_scoda.md` |
| 104 | 작업 | P77 — 모든 Canonical DB 파일명에 버전 포함 | `20260301_104_P77_versioned_db_filename.md` |
| 105 | 작업 | P78: Treatise (2004) Taxonomy Import | `20260301_105_P78_treatise_import.md` |
| 106 | 작업 | P79: Profile Selector UI + Profile-Based Tree | `20260301_106_P79_profile_selector_ui.md` |
| 107 | 작업 | P80: Assertion DB CRUD 적용 | `20260301_107_P80_assertion_crud.md` |

**결과:**
- **Radial Tree (102b/103)**: assertion DB에 radial tree (nodes + edges 쿼리), canonical DB에 radial tree (nodes만, parent_id 포함). scoda-engine에 d3.tree layout, subtree root, context menu, collapse/expand 등 대폭 고도화 (+554줄)
- **Versioned DB (104)**: `db/trilobase.db` → `db/trilobase-0.2.6.db`, `db/paleocore.db` → `db/paleocore-0.1.1.db`. `scripts/db_path.py` 헬퍼로 glob 기반 최신 버전 자동 탐색. 스크립트 8개 + 테스트 5개소 교체
- **Treatise Import (105)**: Agnostida/Redlichiida 2개 chapter 임포트. 370 taxa 매칭, 50 taxa 신규 생성 (Subfamily 32개 포함), 421 PLACED_IN assertions. treatise2004 profile edges 5,138개. 17/17 검증
- **Profile Selector (106)**: taxonomy_tree/family_genera/taxon_children 쿼리를 edge_cache 기반으로 전환. global_controls manifest로 profile dropdown 구현. localStorage 저장. assertion v0.1.2
- **CRUD (107)**: taxon/assertion/reference/classification_profile에 editable_entities manifest 선언. admin 모드에서 웹 UI CRUD 가능. inline assertion 편집 지원
- 테스트 112→118 (+6)

---

### 2026-03-02 (Day 26) — 아키텍처 리뷰

| # | 유형 | 제목 | 문서 |
|---|------|------|------|
| R01 | 리뷰 | 시간에 따라 변화하는 Taxonomy 관리 방법 | `20260302_R01_taxonomy_management.md` |
| R02 | 리뷰 | 두 Classification Profile의 시각적 비교 방법 | `20260302_R02_tree_diff_visualization.md` |

**R01 (Taxonomy Management)**: 6가지 아이디어 탐색 — Layered Profile (계층적 프로필 합성), Assertion Timeline (개별 taxon 의견 이력), Revision Package (논문 단위 일괄 import), Working Classification (mutable 현행 분류), Scope-Aware Assertion (범위 인식), Temporal Authority (최신 우선). 단계적 접근 제안: Phase A (effective_year, scope_type, opinion history UI) → Phase B (revision package) → Phase C (layered profile) → Phase D (tree drag-and-drop).

**R02 (Tree Diff Visualization)**: 4가지 비교 표시 모드 설계 — Diff Table (SQL 차이 목록), Diff Tree (색상 코딩 + ghost edge), Side-by-side (동기화된 2-panel), Overlay (같은 캔버스에 겹침), Animated Morphing (좌표 보간 전환). 5-Phase 구현 순서: Compare UI 인프라 → Diff Table → Diff Tree → Overlay+Side-by-side → Animated Morphing. scoda-engine/trilobase 책임 분리 원칙 정립.
...
| 02-28 | Modular Rebuild Pipeline (8-step, 35/35 검증, `scripts/pipeline/`) |
| 03-01 | Versioned DB 파일명 (`trilobase-{ver}.db`, `db_path.py` 헬퍼) |

### 9. Generic Viewer 강화 (02-16 ~ 03-01)
...
| 03-01 | Radial Tree (display: "radial") — radial + rectangular 레이아웃, depth toggle, subtree root, context menu |
| 03-01 | Profile Selector UI — global_controls manifest, localStorage, profile-aware tree |
| 03-01 | Manifest-driven CRUD — editable_entities, admin 모드 웹 UI |
...
| 02-27 | genus_detail Synonymy 렌더링 수정 + fide 133건 추가 매칭 | `20260227_097_synonym_manifest_and_fide_fix.md` |
| 02-28 | country_id 3,769건 수정 (77.8%), formation 350건 수정 | `20260228_098_fix_country_id_and_formation.md` |
| 02-28 | Rebuild pipeline 행 단위 diff 해소 (Fix A~H) | `20260228_100_rebuild_diff_resolution.md` |

### 11. Taxonomic Opinions / Assertion-Centric Model (02-18 ~ 03-01)
...
| 02-27 | Synonyms 1,055건 → taxonomic_opinions 마이그레이션 (SYNONYM_OF) |
| 02-28 | Assertion-centric test DB (P74): taxon + assertion + reference + classification_profile |
| 02-28 | Assertion DB UI 동등화 (P74b): 40 queries, 15 views, junction tables |
| 02-28 | Treatise Ch4/Ch5 taxonomy JSON 추출 |
| 03-01 | Treatise (2004) import: 370 taxa 매칭, 50 신규, 421 PLACED_IN assertions |
| 03-01 | Profile Selector UI: edge_cache 기반 tree, global_controls, localStorage |
| 03-01 | CRUD: taxon/assertion/reference editable_entities manifest |

최종: taxonomic_opinions 1,139건, assertion DB assertions 6,563건, 3 profiles, 118 tests

### 12. Radial Tree / Tree Chart (02-28 ~ 03-01)

| 날짜 | 내용 |
|------|------|
| 02-28 | P75 계획: Radial Taxonomy Tree Visualization |
| 03-01 | Assertion DB radial tree (102b): nodes + edges 쿼리, rank_radius, depth toggle |
| 03-01 | scoda-engine 고도화: d3.tree layout, LCA angular separation, subtree root, context menu, collapse/expand (+554줄) |
| 03-01 | Canonical DB radial tree (103): nodes만 (parent_id 포함), edge 쿼리 불필요 |
| 03-02 | R02: Tree Diff Visualization 설계 (4가지 비교 모드 + 5-Phase 구현 계획) |
...
| 02-16 | P53 | Assertion-Centric Canonical Model (장기 비전) | ✅ 구현 완료 (02-28, P74) |
| 02-16 | P54 | Generic Viewer Auto-Discovery | ✅ 구현 완료 (02-18, #075) |
| 02-17 | P56 | 2026 Q1 로드맵 (Track A/B/C) | 현행 |
| 02-28 | P72 | 소스→DB 완전 재구성 파이프라인 | ✅ 구현 완료 (02-28, #099) |
| 02-28 | P75 | Radial Tree Visualization | ✅ 구현 완료 (03-01, #102b/#103) |
| 03-02 | R01 | 시간에 따른 Taxonomy 관리 (6가지 아이디어) | 아이디어 탐색 |
| 03-02 | R02 | Classification Profile 시각적 비교 (4가지 모드) | 설계 완료, 미구현 |
...
| 유형 | 접두사 | 개수 | 설명 |
|------|--------|------|------|
| 작업 로그 | `NNN` (숫자) | 115 | 완료된 작업의 상세 기록 (001~107, 102b 포함) |
| 계획 문서 | `PNN` | 81 | 작업 전 설계/계획 (P01~P80) |
| 리뷰 문서 | `RNN` | 4 | 아키텍처 검토 (archive R01-R02 + devlog R01-R02) |
...
| 날짜 | 테스트 수 | 주요 변경 |
|------|-----------|-----------|
| 02-20 | 82 | taxon_bibliography junction 테스트 +16 |
| 02-22 | 101 | Agnostida, SPELLING_OF, parent_id NULL 해소 테스트 |
| 02-27 | 108 | SYNONYM_OF 마이그레이션 테스트 +7 |
| 02-28 | 112 | country_id 정합성 테스트 +4 |
| 03-01 | 118 | Assertion DB editable entities 테스트 +6 |

## 버전 이력

| 날짜 | 패키지 | 버전 | 주요 내용 |
|------|--------|------|-----------|
| 02-21 | trilobase | 0.2.0 | Family parent_id 해소, 버전 관리 도입 |
| 02-21 | paleocore | 0.1.1 | 초기 버전 범프 |
| 02-22 | trilobase | 0.2.2 | Taxonomic Opinions 확장, parent_id NULL 전수 해소 |
| 02-24 | trilobase | 0.2.3 | UI 버그 수정, genus_locations 복원 |
| 02-27 | trilobase | 0.2.4 | Synonym → Opinions 마이그레이션, fide 매칭 추가 |
| 02-28 | trilobase | 0.2.5 | country_id/formation 대수정 |
| 02-28 | trilobase | 0.2.6 | Rebuild pipeline, versioned DB 파일명 |
| 02-28 | trilobase-assertion | 0.1.0 | Assertion-centric DB 초기 버전 |
| 03-01 | trilobase-assertion | 0.1.2 | Treatise import, profile selector, CRUD |
