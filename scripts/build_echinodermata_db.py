#!/usr/bin/env python3
"""Build echinodermata DB from Treatise echinoderm source files.

Two classification profiles:
  Profile 1: Treatise 1966-1967 — vol_s + vol_u1 + vol_u2
  Profile 2: Treatise Revised 2011 — vol_t (Crinoidea only)

Note: Profile 2 covers only Crinoidea (Part T), while Profile 1 covers
Echinozoa (Part S) and Asterozoa/Echinozoa (Part U). They cover different
groups, so both profiles include their respective taxa independently.

Pure source-driven build (no canonical DB dependency).

Usage:
    python scripts/build_echinodermata_db.py [--version 0.1.0]
"""
...
PROFILES = [
    {
        "name": "Treatise 1966-1967",
        "description": "Treatise on Invertebrate Paleontology, Parts S & U, Echinodermata 1 & 3 (1966-1967)",
        "sources": [
            "treatise_echinodermata_1967_vol_s.txt",
            "treatise_echinodermata_1966_vol_u1.txt",
            "treatise_echinodermata_1966_vol_u2.txt",
        ],
        "reference": {
            "authors": "MOORE, R.C. (ed.)",
            "year": 1967,
            "title": "Treatise on Invertebrate Paleontology, Parts S & U, Echinodermata 1 & 3",
            "publisher": "Geological Society of America & University of Kansas",
            "reference_type": "book",
            "raw_entry": (
                "Moore, R.C. (ed.), 1966-1967. Treatise on Invertebrate Paleontology, "
                "Part S, Echinodermata 1 (1967) & Part U, Echinodermata 3, Volumes 1-2 (1966)."
            ),
        },
    },
    {
        "name": "Treatise Revised 2011",
        "description": "Treatise on Invertebrate Paleontology, Part T (Revised), Echinodermata 2, Crinoidea (2011)",
        "sources": [
            "treatise_echinodermata_2011_vol_t.txt",
        ],
        "reference": {
            "authors": "HESS, H. & MESSING, C.G. (eds.)",
            "year": 2011,
            "title": "Treatise on Invertebrate Paleontology, Part T (Revised), Echinodermata 2, Crinoidea, Volume 3",
            "publisher": "Paleontological Institute, University of Kansas",
            "reference_type": "book",
            "raw_entry": (
                "Hess, H. & Messing, C.G. (eds.), 2011. Treatise on Invertebrate Paleontology, "
                "Part T (Revised), Echinodermata 2 (Crinoidea), Volume 3."
            ),
        },
    },
]
...
RANK_KEYWORDS = {
    "Phylum", "Subphylum", "Subclass", "Class", "Order", "Suborder",
    "Superfamily", "Family", "Subfamily",
}
RANK_ORDER = {
    "Phylum": -2, "Subphylum": -1,
    "Class": 0, "Subclass": 1, "Order": 2, "Suborder": 3, "Superfamily": 4,
...
def parse_hierarchy_body(body: str, default_leaf_rank="Genus"):
    ...
    for raw_line in body.splitlines():
        ...
        cur_order = RANK_ORDER.get(rank, 7)
...
def create_schema(cur):
    ...
    CREATE TABLE taxon (
        ...
        name TEXT NOT NULL,
        rank TEXT NOT NULL
            CHECK(rank IN ('Phylum','Subphylum','Class','Subclass','Order','Suborder',
    ...
    return {
        ...
        "views": {
            "taxonomy_tree": {
                ...
                "display": "tree",
                "title": "Taxonomy",
                "description": "Echinoderm hierarchical classification (Treatise 1966-1967 & Revised 2011)",
            ...
            "genera_table": {
                "type": "table",
                "title": "Genera",
                "description": "Flat list of all echinoderm genera",
            ...
            "tree_chart": {
                ...
                "display": "tree_chart",
                "title": "Tree",
                "description": "Echinoderm taxonomy tree -- radial or rectangular layout",
                "icon": "bi-diagram-3",
...
def write_scoda_metadata(cur, version, ref_id):
    ...
    """)
    metadata = {
        "artifact_id": "echinodermata",
        "name": "Echinodermata",
        "version": version,
        "description": "Echinoderm genus-level taxonomy from the Treatise (1966-1967 & Revised 2011)",
        "license": "CC-BY-4.0",
        "schema_version": "1.0",
        "created_at": now,
    }
    ...
    """, (
        "publication",
        "Moore, R.C. (ed.), 1966-1967. Treatise on Invertebrate Paleontology, "
        "Part S, Echinodermata 1 & Part U, Echinodermata 3. "
        "Geological Society of America & University of Kansas.",
        "Cystoidea, Blastoidea, Eocrinoidea, Paracrinoidea, Edriasteroidea (Part S); "
        "Asterozoa, Echinozoa, Holothuroidea (Part U). "
    ...
    """, (
        ...
        "Subclass Articulata (post-Paleozoic Crinoidea). "
        "Genus-level classification with type species.",
    ))
    ...
    """)
    descs = [
        ("taxon", None, "Echinoderm taxa from Phylum to Genus"),
        ("taxon", "rank", "Taxonomic rank: Phylum, Subphylum, Class, Subclass, Order, Suborder, Superfamily, Family, Subfamily, Genus"),
    ...
    cur.execute(
        ...
        ("Echinodermata default UI manifest",
    ...
    """, (json.dumps([
        ...
        {"name": "rank", "type": "select", "required": True, "label": "Rank",
         "options": ["Phylum", "Subphylum", "Class", "Subclass", "Order", "Suborder",
                     "Superfamily", "Family", "Subfamily", "Genus"]},
...
def main():
    parser = argparse.ArgumentParser(description='Build echinodermata DB from source')
    parser.add_argument('--version', default=VERSION, help=f'Version (default: {VERSION})')
    args = parser.parse_args()

    version = args.version
    dst_path = DST_DIR / f"echinodermata-{version}.db"
    ...
    print(f"Building echinodermata v{version}")
    ...
    for pi, profile in enumerate(PROFILES, 1):
        ...
        # Bridge orphan roots to Phylum ECHINODERMATA
        phylum_id = resolve_taxon("Echinodermata", "Phylum", conn, taxon_index, new_taxa_cache)
        taxon_index.update(new_taxa_cache)
        ...
        if orphan_roots:
            ...
            """, bridge_edges)
            conn.commit()
            print(f"  Bridge edges to Phylum Echinodermata: {len(bridge_edges)}")
            for oid, oname, orank in orphan_roots:
                print(f"    {orank} {oname} -> Phylum Echinodermata")
    ...
    rank_counts = conn.execute(
        ...
        "WHEN 'Class' THEN 2 WHEN 'Subclass' THEN 3 WHEN 'Order' THEN 4 "
        "WHEN 'Suborder' THEN 5 WHEN 'Superfamily' THEN 6 WHEN 'Family' THEN 7 "
        "WHEN 'Subfamily' THEN 8 WHEN 'Genus' THEN 9 END"
    ).fetchall()
