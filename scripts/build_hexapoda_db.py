#!/usr/bin/env python3
"""Build hexapoda DB from Treatise hexapod source file.

Single classification profile:
  Profile 1: Treatise 1992 — treatise_hexapoda_1992.txt

Pure source-driven build (no canonical DB dependency).

Usage:
    python scripts/build_hexapoda_db.py [--version 0.1.0]
"""
...
PROFILES = [
    {
        "name": "Treatise 1992",
        "description": "Treatise on Invertebrate Paleontology, Part R, Arthropoda 4, Hexapoda (1992)",
        "sources": [
            "treatise_hexapoda_1992.txt",
        ],
        "reference": {
            "authors": "CARPENTER, F.M.",
            "year": 1992,
            "title": "Treatise on Invertebrate Paleontology, Part R, Arthropoda 4, Volumes 3 & 4 (Hexapoda)",
            "publisher": "Geological Society of America & University of Kansas Press",
            "reference_type": "book",
            "raw_entry": (
                "Carpenter, F.M. (ed.), 1992. Treatise on Invertebrate Paleontology, "
                "Part R, Arthropoda 4, Volumes 3 & 4 (Hexapoda). Geological Society "
...
def create_schema(cur):
    ...
    CREATE TABLE taxon (
        ...
        name TEXT NOT NULL,
        rank TEXT NOT NULL
            CHECK(rank IN ('Class','Order','Suborder',
    ...
    return {
        ...
        "views": {
            "taxonomy_tree": {
                ...
                "display": "tree",
                "title": "Taxonomy",
                "description": "Hexapod hierarchical classification (Treatise 1992)",
            ...
            "genera_table": {
                "type": "table",
                "title": "Genera",
                "description": "Flat list of all hexapod genera",
            ...
            "tree_chart": {
                ...
                "display": "tree_chart",
                "title": "Tree",
                "description": "Hexapod taxonomy tree — radial or rectangular layout",
                "icon": "bi-diagram-3",
...
def write_scoda_metadata(cur, version, ref_id):
    ...
    """)
    metadata = {
        "artifact_id": "hexapoda",
        "name": "Hexapoda",
        "version": version,
        "description": "Hexapod (insect) genus-level taxonomy from the Treatise (1992)",
        "license": "CC-BY-4.0",
        "schema_version": "1.0",
        "created_at": now,
    }
    ...
    """, (
        ...
        "Part R, Arthropoda 4, Volumes 3 & 4 (Hexapoda). Geological Society "
        "of America and University of Kansas.",
        "Class Insecta: Subclass Apterygota and Subclass Pterygota. "
    ...
    """)
    descs = [
        ("taxon", None, "Hexapod taxa from Class to Genus"),
...
def main():
    parser = argparse.ArgumentParser(description='Build hexapoda DB from source')
    parser.add_argument('--version', default=VERSION, help=f'Version (default: {VERSION})')
    args = parser.parse_args()

    version = args.version
    dst_path = DST_DIR / f"hexapoda-{version}.db"
    ...
    print(f"Building hexapoda v{version}")
    ...
    for pi, profile in enumerate(PROFILES, 1):
        ...
        # Bridge orphan roots to Class INSECTA
        insecta_id = resolve_taxon("Insecta", "Class", conn, taxon_index, new_taxa_cache)
        taxon_index.update(new_taxa_cache)
        ...
        """, (profile_id, insecta_id, profile_id)).fetchall()

        if orphan_roots:
            bridge_edges = [(oid, insecta_id) for oid, _, _ in orphan_roots]
            cur.executemany(f"""
                INSERT OR IGNORE INTO classification_edge_cache (profile_id, child_id, parent_id)
                VALUES ({profile_id}, ?, ?)
            """, bridge_edges)
            conn.commit()
            print(f"  Bridge edges to Class INSECTA: {len(bridge_edges)}")
            for oid, oname, orank in orphan_roots:
                print(f"    {orank} {oname} -> Class INSECTA")
    ...
    rank_counts = conn.execute(
        ...
        "CASE rank WHEN 'Class' THEN 0 WHEN 'Order' THEN 1 WHEN 'Suborder' THEN 2 "
        "WHEN 'Superfamily' THEN 3 WHEN 'Family' THEN 4 "
        "WHEN 'Subfamily' THEN 5 WHEN 'Genus' THEN 6 END"
    ).fetchall()
