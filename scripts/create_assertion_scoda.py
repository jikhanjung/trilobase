#!/usr/bin/env python3
"""Create a .scoda package from the assertion-centric test DB.

Usage:
  python scripts/create_assertion_scoda.py
  python scripts/create_assertion_scoda.py --dry-run
"""
...
def generate_hub_manifest(scoda_path, db_path):
    ...
    package_id = meta.get('artifact_id', 'trilobase-assertion')
...
def main():
    ...
    if args.output:
        ...
        output_path = os.path.abspath(
            os.path.join(DEFAULT_OUTPUT_DIR, f'trilobase-assertion-{version}.scoda'))

    if not os.path.exists(db_path):
        print(f"Error: Database not found: {db_path}", file=sys.stderr)
        print("Run 'python scripts/create_assertion_db.py' first.", file=sys.stderr)
        sys.exit(1)
    ...
    if args.dry_run:
        ...
        manifest = {
            "format": "scoda",
            "format_version": "1.0",
            "name": db_meta.get('artifact_id', 'trilobase-assertion'),
