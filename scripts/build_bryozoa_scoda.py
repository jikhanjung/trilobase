#!/usr/bin/env python3
"""Create a .scoda package from the bryozoa DB.

Usage:
  python scripts/build_bryozoa_scoda.py
  python scripts/build_bryozoa_scoda.py --dry-run
"""
...
_BRYOZOA_RE = re.compile(r'^bryozoa-(\d+\.\d+\.\d+)\.db$')


def find_bryozoa_db():
    candidates = glob.glob(os.path.join(DB_DIR, 'bryozoa-*.db'))
    versioned = []
    for path in candidates:
        m = _BRYOZOA_RE.search(os.path.basename(path))
        if m:
            parts = tuple(int(x) for x in m.group(1).split('.'))
            versioned.append((parts, path))
    if not versioned:
        raise FileNotFoundError("No bryozoa-*.db found in db/")
    versioned.sort()
...
def generate_hub_manifest(scoda_path, db_path):
    ...
    package_id = meta.get('artifact_id', 'bryozoa')
...
def main():
    parser = argparse.ArgumentParser(description='Create a .scoda package from the bryozoa DB')
    parser.add_argument('--db', default=None, help='Path to bryozoa DB')
    parser.add_argument('--output', default=None, help='Output .scoda file path')
    parser.add_argument('--dry-run', action='store_true', help='Preview without creating')
    args = parser.parse_args()

    db_path = os.path.abspath(args.db) if args.db else find_bryozoa_db()
    if args.output:
        ...
        output_path = os.path.abspath(
            os.path.join(DEFAULT_OUTPUT_DIR, f'bryozoa-{version}.scoda'))

    if not os.path.exists(db_path):
        print(f"Error: Database not found: {db_path}", file=sys.stderr)
        print("Run 'python scripts/build_bryozoa_db.py' first.", file=sys.stderr)
        sys.exit(1)
