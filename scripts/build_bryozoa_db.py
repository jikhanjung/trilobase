#!/usr/bin/env python3
"""Build bryozoa DB from Treatise bryozoan source files.

Two classification profiles:
  Profile 1: Treatise 1953 (original)
  Profile 2: Treatise Revised 1983

Pure source-driven build (no canonical DB dependency).

Usage:
    python scripts/build_bryozoa_db.py [--version 0.1.0]
"""
...
PROFILES = [
    {
        "name": "Treatise 1953",
        "description": "Treatise on Invertebrate Paleontology, Part G, Bryozoa (1953)",
        "sources": [
            "treatise_bryozoa_1953.txt",
        ],
        "reference": {
            "authors": "BASSLER, R.S.",
            "year": 1953,
            "title": "Treatise on Invertebrate Paleontology, Part G, Bryozoa",
            "publisher": "Geological Society of America & University of Kansas Press",
            "reference_type": "book",
            "raw_entry": (
                "Bassler, R.S., 1953. Treatise on Invertebrate Paleontology, "
                "Part G, Bryozoa. Geological Society of America & University of Kansas Press."
            ),
        },
    },
    {
        "name": "Treatise Revised 1983",
        "description": "Treatise on Invertebrate Paleontology, Part G (Revised), Bryozoa (1983)",
        "sources": [
            "treatise_bryozoa_revised_1983.txt",
        ],
        "reference": {
            "authors": "BOARDMAN, R.S., CHEETHAM, A.H., BLAKE, D.B., UTGAARD, J., KARKLINS, O.L., COOK, P.L., SANDBERG, P.A., LUTAUD, G. & WOOD, T.S.",
            "year": 1983,
            "title": "Treatise on Invertebrate Paleontology, Part G (Revised), Bryozoa",
            "publisher": "Geological Society of America & University of Kansas",
            "reference_type": "book",
            "raw_entry": (
                "Boardman, R.S., Cheetham, A.H., Blake, D.B., Utgaard, J., Karklins, O.L., "
                "Cook, P.L., Sandberg, P.A., Lutaud, G., & Wood, T.S., 1983. "
                "Treatise on Invertebrate Paleontology, Part G (Revised), Bryozoa. "
    ...
    return {
        ...
        "views": {
            "taxonomy_tree": {
                ...
                "display": "tree",
                "title": "Taxonomy",
                "description": "Bryozoan hierarchical classification (Treatise 1953 & Revised 1983)",
            ...
            "genera_table": {
                "type": "table",
                "title": "Genera",
                "description": "Flat list of all bryozoan genera",
            ...
            "tree_chart": {
                ...
                "display": "tree_chart",
                "title": "Tree",
                "description": "Bryozoan taxonomy tree -- radial or rectangular layout",
                "icon": "bi-diagram-3",
...
def write_scoda_metadata(cur, version, ref_id):
    ...
    metadata = {
        ...
        "name": "Bryozoa",
        "version": version,
        "description": "Bryozoan genus-level taxonomy from the Treatise (1953 & Revised 1983)",
        "license": "CC-BY-4.0",
        "schema_version": "1.0",
        "created_at": now,
    }
    ...
    """, (
        ...
        "Bassler, R.S., 1953. Treatise on Invertebrate Paleontology, "
        "Part G, Bryozoa. Geological Society of America & University of Kansas Press.",
        "Phylum Bryozoa: Class Gymnolaemata and Class Phylactolaemata. "
    ...
    """, (
        "publication",
        "Boardman, R.S., Cheetham, A.H., Blake, D.B. & others, 1983. "
        "Treatise on Invertebrate Paleontology, Part G (Revised), Bryozoa. "
        "Geological Society of America & University of Kansas.",
        "Order Cystoporata and Order Cryptostomata. "
        "Genus-level classification with type species.",
    ))
    ...
    """)
    descs = [
        ("taxon", None, "Bryozoan taxa from Phylum to Genus"),
        ("taxon", "rank", "Taxonomic rank: Phylum, Class, Order, Suborder, Family, Subfamily, Genus"),
    ...
    """, (json.dumps([
        ...
        {"name": "rank", "type": "select", "required": True, "label": "Rank",
         "options": ["Phylum", "Class", "Order", "Suborder",
...
def main():
    parser = argparse.ArgumentParser(description='Build bryozoa DB from source')
    parser.add_argument('--version', default=VERSION, help=f'Version (default: {VERSION})')
    args = parser.parse_args()

    version = args.version
    dst_path = DST_DIR / f"bryozoa-{version}.db"
    ...
    print(f"Building bryozoa v{version}")
    ...
    for pi, profile in enumerate(PROFILES, 1):
        ...
        # Bridge orphan roots to Phylum BRYOZOA
        phylum_id = resolve_taxon("Bryozoa", "Phylum", conn, taxon_index, new_taxa_cache)
        taxon_index.update(new_taxa_cache)
        ...
        if orphan_roots:
            ...
            """, bridge_edges)
            conn.commit()
            print(f"  Bridge edges to Phylum BRYOZOA: {len(bridge_edges)}")
            for oid, oname, orank in orphan_roots:
                print(f"    {orank} {oname} -> Phylum BRYOZOA")
