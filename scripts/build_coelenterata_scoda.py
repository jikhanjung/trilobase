#!/usr/bin/env python3
"""Create a .scoda package from the coelenterata DB.

Usage:
  python scripts/build_coelenterata_scoda.py
  python scripts/build_coelenterata_scoda.py --dry-run
"""
...
_COELENTERATA_RE = re.compile(r'^coelenterata-(\d+\.\d+\.\d+)\.db$')


def find_coelenterata_db():
    candidates = glob.glob(os.path.join(DB_DIR, 'coelenterata-*.db'))
    versioned = []
    for path in candidates:
        m = _COELENTERATA_RE.search(os.path.basename(path))
        if m:
            parts = tuple(int(x) for x in m.group(1).split('.'))
            versioned.append((parts, path))
    if not versioned:
        raise FileNotFoundError("No coelenterata-*.db found in db/")
    versioned.sort()
...
def generate_hub_manifest(scoda_path, db_path):
    ...
    package_id = meta.get('artifact_id', 'coelenterata')
...
def main():
    parser = argparse.ArgumentParser(description='Create a .scoda package from the coelenterata DB')
    parser.add_argument('--db', default=None, help='Path to coelenterata DB')
    parser.add_argument('--output', default=None, help='Output .scoda file path')
    parser.add_argument('--dry-run', action='store_true', help='Preview without creating')
    args = parser.parse_args()

    db_path = os.path.abspath(args.db) if args.db else find_coelenterata_db()
    if args.output:
        ...
        output_path = os.path.abspath(
            os.path.join(DEFAULT_OUTPUT_DIR, f'coelenterata-{version}.scoda'))

    if not os.path.exists(db_path):
        print(f"Error: Database not found: {db_path}", file=sys.stderr)
        print("Run 'python scripts/build_coelenterata_db.py' first.", file=sys.stderr)
        sys.exit(1)
